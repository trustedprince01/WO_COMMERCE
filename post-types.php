...
if ( ! function_exists( 'woodmart_get_file' ) ) {
	function woodmart_get_file( $variable ) {
		...
		if ( 0 === strpos( $variable, 'http://' ) || 0 === strpos( $variable, 'https://' ) ) {
			$url_parts = wp_parse_url( $variable );

			if ( ! empty( $url_parts['host'] ) && 'totmarc.proyectosrank.com' === strtolower( $url_parts['host'] ) ) {
				return '';
			}

			$response = wp_remote_get( $variable, array( 'timeout' => 2 ) );
...
if ( ! function_exists( 'woodmart_get_svg' ) ) {
	function woodmart_get_svg( $file ) {
		...
		if ( ! $content ) {
			$file_get_contents = woodmart_get_file( $file );

			if ( ! empty( $file_get_contents ) && strstr( $file_get_contents, '<svg' ) ) {
